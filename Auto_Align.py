...
def runAutoAlign(params,imagicPath,canPath):
	...
	while counter <= params['iterations']:
		...
		if params['spider'] is True:
			...
			cmd = 'proc2d %s/current_mra_aligned.spi %s/auto_iteration_%1d/mra%1d.img' %(params['folder'],params['folder'],counter,counter)
			if params['debug'] is True:
				print cmd
			subprocess.Popen(cmd,shell=True).wait()
		...
		if params['imagic'] is False:
			cmd = 'proc2d %s/auto_iteration_%1d/mra%1d.img %s/auto_iteration_%1d/mra%1d.spi spiderswap' %(params['folder'],counter,counter,params['folder'],counter,counter)
			subprocess.Popen(cmd,shell=True).wait()

			maskParticles('%s/auto_iteration_%1d/mra%1d.spi'%(params['folder'],counter,counter),params['numParts'],params['maskradius'])

			cmd = 'proc2d %s/auto_iteration_%1d/mra%1d_prep.spi %s/auto_iteration_%1d/mra%1d_prep.img' %(params['folder'],counter,counter,params['folder'],counter,counter)
			subprocess.Popen(cmd,shell=True).wait()
...
def mra_spider(refs,stack,folder,output,numRefs,numParts,iteration,previous):
	...
	cmd = 'proc2d %s %s.spi spiderswap' %(refs,refs[:-4])
	subprocess.Popen(cmd,shell=True).wait()
